...
#define SIZE 256            //intial size of malloc
#define MAX_REDIRECTION 10  //maximum of redirection
 ...
 * \param length the length of replied buffer
 * \return a pointer to the first data byte
 */
char *read_http_reply(struct http_reply *reply, int* length);

/**
 * Store the received reply dara in the reply->reply_buffer.
 */
